...
export default function useFetchSingleEvent(event_id) {
  ...
  useEffect(() => {
    async function fetchEvent(event_id) {
      try {
        setIsLoading(true);
        const responseEvent = await fetch(`http://3.8.100.175:8080/encore/events/${event_id}`);
        if (!responseEvent.ok) {
          throw new Error();
        }
        const event = await responseEvent.json();
        const venue = venues.find((venue) => venue.name === event.venue);
        const responseBio = await fetch(`http://3.8.100.175:8080/encore/artists/${event.artist_id}`);
        if (!responseBio.ok) {
          throw new Error();
        }
        const artist = await responseBio.json();
        const responseOtherEvents = await fetch(`http://3.8.100.175:8080/encore/events?city=All&dateFrom=&dateTo=&artist_id=${event.artist_id}`);
