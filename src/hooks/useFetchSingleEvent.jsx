...
export default function useFetchSingleEvent(event_id) {
  ...
  useEffect(() => {
    async function fetchEvent(event_id) {
      try {
        setIsLoading(true);
        const responseEvent = await fetch(`http://localhost:3000/encore/events/${event_id}`);
        if (!responseEvent.ok) {
          throw new Error();
        }
        const event = await responseEvent.json();
        const venue = venues.find((venue) => venue.name === event.venue);
        const responseBio = await fetch(`http://localhost:3000/encore/artists/${event.artist_id}`);
        if (!responseBio.ok) {
          throw new Error();
        }
