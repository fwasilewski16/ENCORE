...
export default function AllEvents(props: AllEventsProps): JSX.Element {
  return (
    <div className="mb-3">
      ...
      {!props.isLoading && props.events.map((event: Event): JSX.Element => <EventSingleComponent name={event.artist_name} event_id={event.event_id} key={event._id} date={event.date} img={event.img} venue={event.venue} city={event.city} function={"events"} />)}
    </div>
  );
}
